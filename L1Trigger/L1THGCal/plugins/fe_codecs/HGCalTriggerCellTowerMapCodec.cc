...
DEFINE_EDM_PLUGIN(HGCalTriggerFECodecFactory, 
        HGCalTriggerCellTowerMapCodec,
        "HGCalTriggerCellTowerMapCodec");

HGCalTriggerCellTowerMapCodec::
HGCalTriggerCellTowerMapCodec(const edm::ParameterSet& conf) : Codec(conf),
    codecImpl_(conf)
{
}
        ...
        const HGCBHDigiCollection& bh) 
{
    ...
    // select highest trigger cell in tower map and assign full hwPt of tower map   
    codecImpl_.towerMapSelect(data_);
}
...
setDataPayloadImpl(const l1t::HGCFETriggerDigi& digi)
{
    ...
    HGCalTriggerCellTowerMapCodec codecInput(conf);
...
std::vector<bool>
HGCalTriggerCellTowerMapCodec::
encodeImpl(const HGCalTriggerCellTowerMapCodec::data_type& data) const 
{
    return codecImpl_.encode(data, *geometry_);
}

HGCalTriggerCellTowerMapCodec::data_type
HGCalTriggerCellTowerMapCodec::
